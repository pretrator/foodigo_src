        ...
        if ($this->request->request['razorpay_payment_id']) {
            ...
            if ($success === true) {
                if (!$order_info['order_status_id']) {
                    $this->model_checkout_order->addOrderHistory($merchant_order_id, 26, 'Payment Successful. Razorpay Payment Id:' . $razorpay_payment_id, true);
                    $this->db->query("update foodigo.oc_order_product set status=0 where order_id=".$merchant_order_id);
                ...
                if (!$order_info['order_status_id']) {
                    $this->model_checkout_order->confirm($merchant_order_id, '27', $error . ' Payment Failed! Check Razorpay dashboard for details of Payment Id:' . $razorpay_payment_id, true);
                } else {
                    $this->model_checkout_order->update($merchant_order_id, '27', $error . ' Payment Failed! Check Razorpay dashboard for details of Payment Id:' . $razorpay_payment_id, true);
                }
