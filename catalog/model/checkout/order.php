	...
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "', manufac_id = (select manufacturer_id from foodigo.oc_product where product_id=".(int)$product['product_id'].")");
                ...
                $this->db->query("insert into oc_integrate values (".(int)$order_id.",".(int)$product['product_id'].",(select manufacturer_id from oc_product where product_id=".(int)$product['product_id']."),0)");
			}
		}
